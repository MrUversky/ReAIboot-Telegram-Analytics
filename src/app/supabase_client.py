...
class SupabaseClient:
    ...
    def _initialize_client(self):
        ...
        try:
            if not SUPABASE_AVAILABLE:
                logger.warning(
                    "Supabase library not available, running without database"
                )
                return
            ...
            self.client = create_client(
                supabase_url=settings.supabase_url, supabase_key=api_key
            )

            key_type = (
                "service role" if settings.supabase_service_role_key else "anonymous"
            )
            logger.info(
                f"Supabase client initialized successfully (using {key_type} key)"
            )
    ...
    def upsert_channel(
        self, username: str, title: str = None, description: str = None
    ) -> bool:
        ...
        try:
            ...
            if not existing_channel:
                ...
                if existing_legacy.data:
                    ...
                    logger.info(
                        f"Found legacy channel {existing_channel['username']}, updating to canonical format"
                    )
            ...
            if existing_channel:
                ...
                logger.info(
                    f"Updated existing channel {existing_channel['username']} with title '{title}'"
                )
                ...
                logger.info(
                    f"Created new channel {canonical_username} with title '{title}'"
                )
                return len(result.data) > 0
    ...
    def update_channel_last_parsed(self, username: str) -> bool:
        ...
        try:
            result = (
                self.client.table("channels")
                .update({"last_parsed": datetime.utcnow().isoformat()})
    ...
    def is_post_processed(
        self, message_id: int, channel_username: str, analysis_type: str = None
    ) -> bool:
        ...
        try:
            ...
            query = (
                self.client.table("post_analysis").select("id").eq("post_id", post_id)
            )
    ...
    def ensure_channel_exists(
        self, channel_username: str, channel_info: Dict[str, Any] = None
    ) -> bool:
    ...
    def save_posts_batch(
        ...
        try:
            ...
            if channel_username and posts:
                channel_title = channel_info.get("title") if channel_info else None
                channel_description = (
                    channel_info.get("about") if channel_info else None
                )
                self.upsert_channel(
                    channel_username, channel_title, channel_description
                )
            ...
            for post in posts:
                logger.info(
                    f"Подготавливаем пост для сохранения: message_id={post.get('message_id')}, channel={post.get('channel_username')}"
                )
                ...
                if "date" not in post:
                    logger.error(
                        f"Поле date отсутствует в посте {post.get('message_id', 'unknown')}"
                    )
                    continue
                ...
                post_data = {
                    ...
                    if post["channel_username"].startswith("@")
                    else f"@{post['channel_username']}",
            ...
            if posts_data:
                ...
                logger.error(
                    f"Channel username in post: {posts_data[0].get('channel_username')}"
                )
            return 0
    ...
    def save_post_analysis(self, analysis_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = (
                self.client.table("post_analysis")
    ...
    def get_post_analysis(
        self, post_id: str, analysis_type: str = None
        ...
        try:
            query = (
                self.client.table("post_analysis").select("*").eq("post_id", post_id)
            )
    ...
    def start_parsing_session(
        self, channels_count: int, user_id: str = None
    ) -> Optional[int]:
        ...
        try:
            ...
            result = (
                self.client.table("parsing_sessions").insert(session_data).execute()
            )
    ...
    def get_parsing_session(self, session_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("parsing_sessions")
                .select("*")
                .eq("id", session_id)
                .execute()
            )
    ...
    def update_parsing_session(
        self, session_id: int, update_data: Dict[str, Any]
    ) -> bool:
    ...
    def get_channel_by_id(self, channel_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("channels").select("*").eq("id", channel_id).execute()
            )
            return result.data[0] if result.data else None
        except Exception as e:
    ...
    def get_rubric_formats(self, rubric_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            query = self.client.table("rubric_formats").select(
                "*, rubrics(*), reel_formats(*)"
            )
    ...
    def update_system_setting(
        ...
        try:
            update_data = {"value": value, "updated_at": datetime.now().isoformat()}
            if description:
                update_data["description"] = description

            self.client.table("system_settings").update(update_data).eq(
                "key", key
            ).execute()
            return True
        except Exception as e:
    ...
    def get_channel_baseline(self, channel_username: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("channel_baselines")
                .select("*")
    ...
    def save_channel_baseline(self, baseline_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.client.table("channel_baselines").upsert(
                baseline_data, on_conflict="channel_username"
            ).execute()
            return True
        except Exception as e:
    ...
    def get_channels_needing_baseline_update(
        ...
        try:
            ...
            result = (
                ...
                .or_(f"last_calculated.lt.{cutoff_time},last_calculated.is.null")
                .execute()
            )
    ...
    def update_post_viral_metrics(
        self, post_id: str, viral_data: Dict[str, Any]
    ) -> bool:
    ...
    def get_viral_posts(
        ...
        try:
            if min_viral_score <= 0:
                # Если min_viral_score <= 0, получаем все посты и фильтруем на клиенте
                query = self.client.table("posts").select("*")
                if channel_username:
                    query = query.eq("channel_username", channel_username)

                result = query.limit(limit * 2).execute()  # Берем больше для фильтрации

                # Фильтруем на клиенте посты с не-null viral_score
                if result.data:
                    filtered_posts = [
                        post
                        for post in result.data
                        if post.get("viral_score") is not None
                    ]
                    # Сортируем по viral_score desc
                    filtered_posts.sort(
                        key=lambda x: x.get("viral_score", 0), reverse=True
                    )
                    return filtered_posts[:limit]
                else:
                    return []
            else:
                # Иначе только посты с viral_score >= min_viral_score
                query = (
                    self.client.table("posts")
                    .select("*")
