...
# Обновляем промпт для анализа 20 постов
...
⚠️ ВАЖНО: Проанализируй ВСЕ предоставленные посты. Дай МАКСИМАЛЬНО КОНКРЕТНЫЕ рекомендации. После прочтения анализа я должен суметь сразу создать 3-5 постов для своего канала.'''

result = client.client.table('llm_prompts').update({'user_prompt': new_user_prompt}).eq('name', 'viral_trends_analysis').execute()
print('Prompt updated successfully for 20 posts analysis')
